...
const exportCategories = [
  { value: "students", label: "ข้อมูลนักศึกษา", icon: Users, fields: ["รหัสนักศึกษา", "ชื่อ-นามสกุล", "คณะ", "สาขา", "ชั้นปี", "GPA", "สถานะ", "อาจารย์ที่ปรึกษา"] },
  { value: "teachers", label: "ข้อมูลอาจารย์", icon: GraduationCap, fields: ["รหัสอาจารย์", "ชื่อ-นามสกุล", "ตำแหน่งวิชาการ", "สาขา", "อีเมล", "เบอร์โทร", "ตำแหน่งบริหาร"] },
  { value: "courses", label: "ข้อมูลรายวิชา", icon: BookOpen, fields: ["รหัสวิชา", "ชื่อวิชา", "หน่วยกิต", "ผู้สอน", "หลักสูตร", "จำนวนนักศึกษา", "CLO"] },
  { value: "projects", label: "ข้อมูลโครงการ", icon: FolderKanban, fields: ["รหัสโครงการ", "ชื่อโครงการ", "ผู้รับผิดชอบ", "งบประมาณ", "สถานะ", "PLO/YLO"] },
];
...
export default function ExportData() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast({ title: "ส่งออกสำเร็จ", description: `ดาวน์โหลดไฟล์เรียบร้อยแล้ว` });
    } catch (error) {
      toast({ title: "เกิดข้อผิดพลาด", description: "ไม่สามารถส่งออกข้อมูลได้", variant: "destructive" });
    }
  };
    ...
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-1 space-y-4">
          <Card>
            <CardHeader><CardTitle className="text-base">เลือกประเภทข้อมูล</CardTitle></CardHeader>
            <CardContent className="space-y-2">
              {exportCategories.map((category) => (
                <div
                  key={category.value}
                  className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all ${
                    selectedCategory === category.value ? "bg-primary/10 border border-primary" : "bg-muted/30 hover:bg-muted/50"
                  }`}
                  onClick={() => { setSelectedCategory(category.value); setSelectedFields([]); }}
                >
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label className="flex items-center gap-2"><Calendar className="h-4 w-4" /> ปีการศึกษา</Label>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card>
            <CardHeader><CardTitle className="text-base">รูปแบบไฟล์</CardTitle></CardHeader>
            <CardContent>
              <RadioGroup value={format} onValueChange={setFormat} className="flex gap-4">
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="xlsx" id="xlsx" disabled />
                  <Label htmlFor="xlsx" className="flex items-center gap-2 cursor-not-allowed opacity-50">
