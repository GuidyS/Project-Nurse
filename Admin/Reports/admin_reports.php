        ...
        $response = [
            ...
            "data" => [
                "stats" => [
                    ...
                    "downloaded" => number_format($total_budget), 
                    "pending" => 0, 
                    "errors" => 0
                ],
                "templates" => [
                    ...
                    //  อัปเดต: เปลี่ยนชื่อและจุดประสงค์รายงาน ไม่ยุ่งกับเกรดรายวิชา 
                    ["id" => "3", "name" => "รายงานภาพรวมผลการเรียน (GPA)", "description" => "รายชื่อนักศึกษาที่ต้องเฝ้าระวังผลการเรียน (GPA < 2.00)", "category" => "การศึกษา", "iconName" => "BookOpen", "lastGenerated" => date("Y-m-d")]
                ]
            ]
        ];
...
if ($action === 'generate') {
    ...
    //  รายงาน: สรุปงบแผนคณะฯ ตามยุทธศาสตร์
    if (strpos($reportName, 'ยุทธศาสตร์') !== false) {
        ...
        fputs($output, "\xEF\xBB\xBF"); // เติม BOM กันบั๊กภาษาไทยใน Excel
    ...
    //  รายงาน: ภาพรวมผลการเรียน (เฉพาะเด็กที่ต้องเฝ้าระวังเพื่อไม่ละเมิด PDPA)
    if (strpos($reportName, 'ภาพรวมผลการเรียน') !== false) {
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="Report_Academic_Watchlist.csv"');
        $output = fopen('php://output', 'w');
        fputs($output, "\xEF\xBB\xBF"); 

        fputcsv($output, ['รหัสนักศึกษา', 'ชื่อ-สกุล', 'เกรดเฉลี่ยสะสม (GPA)', 'สถานะการเฝ้าระวัง']);

        // Query ค้นหาเฉพาะเด็กที่เกรดรวมต่ำกว่า 2.00 (ไม่โชว์เกรดเด็กคนอื่นเพื่อความเป็นส่วนตัว)
        $sql = "
            SELECT 
                student_id, 
                ...
                cumulative_gpa 
            FROM student 
            WHERE cumulative_gpa < 2.00 AND cumulative_gpa > 0
            ORDER BY cumulative_gpa ASC
        ";
        $stmt = $pdo->query($sql);
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            fputcsv($output, [
                $row['student_id'], 
                $row['full_name'], 
                number_format($row['cumulative_gpa'], 2),
                'ต้องพบอาจารย์ที่ปรึกษา'
            ]);
        }
        fclose($output);
        exit();
