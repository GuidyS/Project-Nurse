...
try {
    ...
    if ($action_type === 'all') {
        ...
        $sql = "UPDATE avisor_notifications SET is_read = 1 WHERE user_id = :user_id AND is_read = 0";
        $stmt = $db->prepare($sql);
        ...
        $sql = "UPDATE avisor_notifications SET is_read = 1 WHERE notification_id = :noti_id AND user_id = :user_id";
        $stmt = $db->prepare($sql);
        $stmt->execute([
