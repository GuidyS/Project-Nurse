...
function licenseReminderEnsureSchema(PDO $db): void
{
    ...
    if (!licenseReminderColumnExists($db, 'faculty', 'license_image')) {
        throw new RuntimeException('ไม่พบคอลัมน์ faculty.license_image กรุณาติดตั้ง schema ที่รองรับก่อนอัปโหลดใบประกอบวิชาชีพ');
    }

    if (!licenseReminderTableExists($db, 'faculty_license_reminders')) {
        throw new RuntimeException('ไม่พบตาราง faculty_license_reminders กรุณาติดตั้ง schema ที่รองรับก่อนใช้งานระบบแจ้งเตือนใบประกอบวิชาชีพ');
    }
