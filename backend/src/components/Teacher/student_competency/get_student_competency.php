...
try {
    ...
    if (!$framework) {
        ...
        echo json_encode(["status" => "error", "message" => "ไม่พบหลักสูตรในระบบ"], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    $itemStmt = $db->prepare("
        ...
        WHERE (cp.framework_id = :fid OR ci.plo_id IS NULL) AND ci.year_level = :yl
    ...
    if (!empty($unassignedItems)) {
        array_unshift($groups, [
            ...
            "plo_name" => "(ข้อประเมินที่ไม่ระบุ PLO)",
