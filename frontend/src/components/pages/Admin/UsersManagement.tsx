...
import { CalendarDays, Loader2, Search, Edit, Trash2, MoreHorizontal, UserPlus, Upload } from "lucide-react";
  ...
  return (
    <>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="modern-scrollbar max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-4 py-4 text-sm">
            ...
            {(editingUserRole === 1 || editingUserRole === 2) && (
              <>
                ...
                <div className="space-y-2"><Label>ว/ด/ป เกิด</Label><DateInputWithIcon value={detailForm.birth_date || ""} onChange={e => setDetailForm({...detailForm, birth_date: e.target.value})} /></div>
                ...
                <div className="space-y-2"><Label>วันหมดอายุใบอนุญาต</Label><DateInputWithIcon value={detailForm.license_expiry || ""} onChange={e => setDetailForm({...detailForm, license_expiry: e.target.value})} /></div>
                <div className="space-y-2"><Label>วันที่เริ่มปฏิบัติงาน</Label><DateInputWithIcon value={detailForm.start_work_date || ""} onChange={e => setDetailForm({...detailForm, start_work_date: e.target.value})} /></div>
                <div className="space-y-2"><Label>วันที่รับตำแหน่งทางวิชาการ</Label><DateInputWithIcon value={detailForm.academic_position_date || ""} onChange={e => setDetailForm({...detailForm, academic_position_date: e.target.value})} /></div>
            ...
            {editingUserRole === 3 && (
              <>
                ...
                <div className="space-y-2"><Label>วันที่จบการศึกษา</Label><DateInputWithIcon value={detailForm.graduation_date || ""} onChange={e => setDetailForm({...detailForm, graduation_date: e.target.value})} /></div>
                <div className="space-y-2"><Label>วันที่พ้นสภาพ</Label><DateInputWithIcon value={detailForm.dropout_date || ""} onChange={e => setDetailForm({...detailForm, dropout_date: e.target.value})} /></div>
