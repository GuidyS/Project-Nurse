...
export default function CLOManagement() {
  ...
  const fetchCLOData = async () => {
    try {
      const response = await api.get(`/index.php?page=get-clo-management&subject_code=${subjectCode}`);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) resetForm(); }}>
            ...
            {/* กันกล่องปิดเองเวลาคลิกนอก dropdown PLO — ปิดได้ด้วยปุ่มยกเลิก/บันทึกเท่านั้น */}
