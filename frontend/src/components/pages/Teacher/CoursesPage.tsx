...
const CoursesPage = () => {
  ...
  const fetchCourses = useCallback(async () => {
    setLoading(true);
    try {
      const response = await api.get("/index.php?page=get_courses");
  ...
  const openGradeDialog = async (course: Course, viewMode: boolean = false) => {
    ...
    try {
      const response = await api.get(`/index.php?page=get_students&course_id=${course.id}`);
      if (response.data && response.data.status === "success") {
  ...
  const saveGrade = async (studentId: number) => {
    ...
    try {
      const response = await api.post("/index.php?page=update_grade", {
        id: studentId,
